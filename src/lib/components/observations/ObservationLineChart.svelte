...
<script lang="ts">
	...
	let brushMin: number | undefined = $state();
	let brushMax: number | undefined = $state();
	...
	let referenceRange = $derived(getReferenceRange(observations[0]));
	let minObsValue = $derived(
		observations.reduce(
			(min, obs) => ((obs.valueQuantity?.value || 0) < min ? obs.valueQuantity?.value || 0 : min),
			Infinity
		)
	);
	let maxObsValue = $derived(
		observations.reduce(
			(max, obs) => ((obs.valueQuantity?.value || 0) > max ? obs.valueQuantity?.value || 0 : max),
			-Infinity
		)
	);
	let minValue = $derived(Math.min(referenceRange?.low || minObsValue, minObsValue));
	let maxValue = $derived(Math.max(referenceRange?.high || maxObsValue, maxObsValue));
	let diff = $derived(maxValue - minValue);
	let yDomainMin = $derived(minValue - diff * 0.1);
	let yDomainMax = $derived(maxValue + diff * 0.1);
	let xDomainDiff = $derived(chartData[chartData.length - 1].timestamp - chartData[0].timestamp);
	let xDomainMinBrush = $derived(chartData[0].timestamp - xDomainDiff * 0.025);
	let xDomainMaxBrush = $derived(chartData[chartData.length - 1].timestamp + xDomainDiff * 0.025);
	let xDomainMin = $derived(
		Math.max(chartData[0].timestamp - xDomainDiff * 0.025, brushMin ?? -Infinity)
	);
	let xDomainMax = $derived(
		Math.min(chartData[chartData.length - 1].timestamp + xDomainDiff * 0.025, brushMax ?? Infinity)
	);
</script>
		...
		<div class="mt-4 flex items-center gap-4 text-sm">
			{#if referenceRange}
				<div class="flex items-center gap-2">
					<div class="h-0.5 w-4 border border-dashed border-slate-400"></div>
